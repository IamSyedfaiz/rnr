...
<body>
    ...
    if ('{{ $task->name }}' == 'Start') {
        ...
        editor.addNode(0, '{{ $task->name }}', 0, 1, {
                {
                    $task - > pos_x
                }
            }, {
        ...
        editor.addNode(0, '{{ $task->name }}', 1, 0, {
                {
                    $task->pos_x
                }
            }, {
        ...
        editor.addNode(0, '{{ $task->name }}', 1, 1, {
                {
                    $task->pos_x
                }
            }, {
    ...
    editor.addNode(0, '{{ $trigger->name }}', 0, 1, {
            {
                $trigger->pos_x
            }
        }, {
    ...
    if (parentNode) { // Check if parentNode exists
        var node = editor.getNodeByData('task_id', {
            {
                $task - > id
            }
        });
        ...
        switch (name) {
            ...
            case '{{ $taskName }}':
                ...
                if ('{{ $taskName }}' == 'Stop') {
                    ...
                    _new_node = `@include('workflows::layouts.task_node_html', [
