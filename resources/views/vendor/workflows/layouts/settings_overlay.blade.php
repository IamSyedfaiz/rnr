        ...
        <div class="row">
            <div class="col-md-12" style="margin-bottom: 20px;">
                <div class="settings-headline">
                    {{ $element }}
                    <h1>{!! $element::$icon !!} {{ $element::getTranslation() }}
                        {{ __('workflows::workflows.Settings') }}</h1>
                </div>
            </div>
            <div class="col-md-12">
                        ...
                        <div class="form-group">
                            <select class="form-control" name="{{ $field }}->type"
                                ...
                                @foreach (config('workflows.data_resources') as $resourceName => $resourceClass)
                                    <option value='{{ $resourceClass }}'
                                        {{ $element->fieldIsSelected($field, $resourceClass) }}>
                                        {{ __('workflows::workflows.Resources.' . $resourceName) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group" id="{{ $field }}">
