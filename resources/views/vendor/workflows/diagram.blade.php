    ...
    <header>
        <div style="width: 250px; ">
            <a href="{{ route('backend.home') }}">
                <h3 class="text-primary ml-5 mt-3">RNR</h3>
            </a>
        </div>
    </header>
    <div class="wrapper">
        ...
        @if (@$workflow)
            @foreach ($workflow->tasks as $task)
                ...
                if ('{{ $task->name }}' == 'Start') {
                    ...
                    editor.addNode(0, '{{ $task->name }}', 0, 1, {{ $task->pos_x }}, {{ $task->pos_y }},
                        '{{ $task->name }}', {
                            task_id: {{ $task->id }},
                            type: 'task'
                        }, new_node);
                    ...
                    editor.addNode(0, '{{ $task->name }}', 1, 0, {{ $task->pos_x }}, {{ $task->pos_y }},
                        '{{ $task->name }}', {
                            task_id: {{ $task->id }},
                            type: 'task'
                        }, new_node);
                    ...
                    editor.addNode(0, '{{ $task->name }}', 1, 1, {{ $task->pos_x }}, {{ $task->pos_y }},
                        '{{ $task->name }}', {
                            task_id: {{ $task->id }},
                            type: 'task'
                        }, new_node);
            ...
            @foreach ($workflow->triggers as $trigger)
                var new_node = `@include('workflows::layouts.task_node_html', [
                    'elementName' => $trigger->name,
                    'element' => $trigger,
                    'type' => 'trigger',
                    'icon' => $trigger::$icon,
                ])`;
                editor.addNode(0, '{{ $trigger->name }}', 0, 1, {{ $trigger->pos_x }}, {{ $trigger->pos_y }},
                    '{{ $trigger->name }}', {
